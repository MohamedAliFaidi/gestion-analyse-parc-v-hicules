...
Vehicule Saisie_Vehicule(Vehicule parc[], int nVehiculesExistants) {
  ...
  do {
    ...
    for (int i = 0; i < nVehiculesExistants; i++)
      if (strcmp(v.immatriculation, parc[i].immatriculation) == 0) {
        printf("Déjà existante\n");
        unique = 0;
        break;
      }
  } while (!unique);
  ...
  do {
    printf("Année: ");
    scanf("%d", &v.annee);
  } while (v.annee < 1900 || v.annee > 2100);
  do {
    printf("Prix/jour: ");
